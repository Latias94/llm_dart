...
void main() {
  group('Ollama v3 parts goldens (contract fixtures)', () {
    Future<void> runFixtureGolden(
      String baseName, {
      String provider = 'ollama',
    }) async {
      final fixturePath = 'test/fixtures/ollama/chat/$baseName.chunks.txt';
      final sessions = [ndjsonStreamFromChunkFile(fixturePath)];
      ...
      for (var i = 0; i < sessions.length; i++) {
        final client = FakeOllamaClient(config)..streamResponse = sessions[i];
    ...
    final metaDir = Directory('test/fixtures/v3_parts/ollama');

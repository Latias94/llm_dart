...
void main() {
  group('DeepSeekChat streaming tool calls', () {
    test('should preserve tool call id across incremental tool_calls',
      ...
      final client = _FakeDeepSeekClient(config);
...
/// Fake DeepSeekClient that returns a synthetic SSE-style stream.
///
/// DeepSeekChat parses "data: ..." lines manually, similar to OpenAI's SSE,
/// so we reuse the same chunk pattern as the OpenAI chat streaming test.
class _FakeDeepSeekClient extends deepseek.DeepSeekClient {
  _FakeDeepSeekClient(deepseek.DeepSeekConfig config) : super(config);

...
void main() {
  group('Anthropic Cache Position Tests', () {
    ...
    test('Official API flexibility - separate cache breakpoints', () {
      ...
      final toolMessage = MessageBuilder.system()
          .tools([testTool])
          .build();
      ...
      final separateBreakpointsMessage = MessageBuilder.system()
          .tools([testTool])
          .anthropicConfig((anthropic) => anthropic.cache(ttl: AnthropicCacheTtl.oneHour)) // Cache tools
          .text('Regular text')
          .text('Additional text')
          .anthropicConfig((anthropic) => anthropic.cache(ttl: AnthropicCacheTtl.fiveMinutes)) // Cache text with different TTL
          .build();

      final data = separateBreakpointsMessage.getExtension<Map<String, dynamic>>('anthropic')!;
      ...
      // Count cache markers
      final cacheMarkers = blocks.where((block) =>
          block is Map<String, dynamic> && 
          block['cache_control'] != null &&
          block['text'] == '').length;
      expect(cacheMarkers, equals(2));

      print('Official API supports separate cache breakpoints for fine-grained control');
    });
    ...
    test('Current implementation behavior summary', () {
      ...
      final expectedContent = 'Text before cache\nText after cache\nText after tools';
      expect(message.content, equals(expectedContent));
      ...
      final hasTools = contentBlocks.any((block) =>
          block is Map<String, dynamic> && block['type'] == 'tools');
      ...
      print('Current implementation: MessageBuilder-level caching includes ALL content');
    });
  });
}

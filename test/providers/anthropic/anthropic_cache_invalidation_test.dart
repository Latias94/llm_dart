  ...
  group('Anthropic Cache Invalidation Tests', () {
    ...
    group('Cache Invalidation Hierarchy', () {
      test('Tool definition changes invalidate entire cache', () {
        ...
        final modifiedTool = Tool.function(
          ...
          parameters: ParametersSchema(
            ...
            properties: {
              'param': ParameterProperty(
                propertyType: 'string',
                description: 'Modified parameter', // Parameter description changed
              ),
            },
            required: ['param'],
          ),
        ...
        final toolsBlock1 = blocks1.firstWhere((b) => (b as Map)['type'] == 'tools') as Map;
        final toolsBlock2 = blocks2.firstWhere((b) => (b as Map)['type'] == 'tools') as Map;
        ...
        expect(tool1Data['description'], isNot(equals(tool2Data['description'])));
      ...
      test('System prompt changes invalidate system and message cache', () {
        ...
        print('System prompt changes invalidate system and message cache - validated');
      });
    ...
    group('Feature Toggle Invalidation', () {
      test('Web search toggle invalidates system cache', () {
        ...
        expect(messageWithoutWebSearch.content, isNot(equals(messageWithWebSearch.content)));
      ...
      test('Citations toggle invalidates system cache', () {
        ...
        expect(messageWithoutCitations.content, isNot(equals(messageWithCitations.content)));
    ...
    group('Cache Breakpoint Limits', () {
      test('Maximum 4 cache breakpoints allowed', () {
        ...
        final message = MessageBuilder.system()
            .text('Section 1')
            .anthropicConfig((anthropic) => anthropic.cache(ttl: AnthropicCacheTtl.oneHour)) // Breakpoint 1
            .text('Section 2')
            .anthropicConfig((anthropic) => anthropic.cache(ttl: AnthropicCacheTtl.oneHour)) // Breakpoint 2
            .text('Section 3')
            .anthropicConfig((anthropic) => anthropic.cache(ttl: AnthropicCacheTtl.fiveMinutes)) // Breakpoint 3
            .text('Section 4')
            .anthropicConfig((anthropic) => anthropic.cache(ttl: AnthropicCacheTtl.fiveMinutes)) // Breakpoint 4
            .text('Final section')
            .build();
        ...
        final cacheBreakpoints = contentBlocks.where((block) =>
      ...
      test('TTL ordering constraint: longer TTL before shorter TTL', () {
        ...
        final correctOrderMessage = MessageBuilder.system()
            ...
            .anthropicConfig((anthropic) => anthropic.cache(ttl: AnthropicCacheTtl.oneHour)) // 1h first
            .text('Short-term content')
            .anthropicConfig((anthropic) => anthropic.cache(ttl: AnthropicCacheTtl.fiveMinutes)) // 5m second
            .build();

        final anthropicData = correctOrderMessage.getExtension<Map<String, dynamic>>('anthropic')!;
        final contentBlocks = anthropicData['contentBlocks'] as List<dynamic>;
        
        final cacheBlocks = contentBlocks
            .where((block) => block is Map<String, dynamic> && block['cache_control'] != null)
            .map((block) => (block as Map<String, dynamic>)['cache_control'])
            .toList();
        
        expect(cacheBlocks.length, equals(2));
        expect(cacheBlocks[0]['ttl'], equals('1h')); // First should be longer TTL
        expect(cacheBlocks[1]['ttl'], equals('5m')); // Second should be shorter TTL

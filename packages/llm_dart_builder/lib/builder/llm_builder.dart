...
class LLMBuilder {
  ...
  ///
  /// The [modelId] should be in the form `"provider:model"` or `"provider/model"`,
  /// for example:
  /// - `"openai:gpt-4o"`
  /// - `"openai/gpt-4o"`
  ...
  LLMBuilder use(String modelId) {
    final colonIndex = modelId.indexOf(':');
    final slashIndex = modelId.indexOf('/');
    final separatorIndex = colonIndex != -1 ? colonIndex : slashIndex;
    if (separatorIndex <= 0 || separatorIndex == modelId.length - 1) {
      throw ArgumentError(
        'Model identifier must be in the form "provider:model" or "provider/model", '
        'e.g. "openai:gpt-4o". '
        'Received: "$modelId".',
      );
    }
  ...
  /// Sets provider-specific extension using a typed key.
  ///
  /// This is a thin wrapper around [extension] that improves type hints
  /// when multiple packages share extension contracts.
  LLMBuilder extensionKey<T>(LLMConfigKey<T> key, T value) {
    return extension(key.name, value);
  }

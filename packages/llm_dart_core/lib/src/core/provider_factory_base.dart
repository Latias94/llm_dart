...
abstract class BaseProviderFactory<T extends Object>
  ...
  E? getExtension<E>(LLMConfig config, String key, [E? defaultValue]) {
    try {
      ...
      // Swallow extension type errors; callers can fall back to defaults.
      return defaultValue;
    }
  }
  ...
  ///
  /// Subclasses should return a strongly-typed default [LLMConfig] (or derive
  /// it from the provider's own configuration type) instead of using Map to
  /// reduce errors.
  @override
...
///
/// This provides additional functionality for providers that use OpenAI's
/// HTTP API shape (chat/completions/embeddings).
abstract class OpenAICompatibleBaseFactory<T extends Object>
...
abstract class LocalProviderFactory<T extends Object>
    extends BaseProviderFactory<T> {
  @override
...
abstract class AudioProviderFactory<T extends Object>
    extends BaseProviderFactory<T> {
  @override
  Set<LLMCapability> get supportedCapabilities => {

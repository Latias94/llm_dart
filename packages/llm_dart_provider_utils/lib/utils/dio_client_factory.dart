...
class DioClientFactory {
  ...
  ///
  /// Priority order:
  /// 1. Custom Dio client (if provided via transportOptions)
  ...
  static Dio create({
    ...
    // Extract custom Dio from config transportOptions
  ...
  static Dio? _extractCustomDio(dynamic config) {
    if (config.originalConfig != null) {
      final original = config.originalConfig as LLMConfig;
      final raw = original.getTransportOption<Object>('customDio');
      if (raw is Dio) return raw;
    }
    return null;
  }

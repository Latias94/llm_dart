...
const int _anthropicDefaultMaxTokens = 1024;
const int _anthropicDefaultThinkingBudgetTokens = 1024;
const String _anthropicInterleavedThinkingBeta =
    'interleaved-thinking-2025-05-14';
const String _anthropicMcpClientBeta = 'mcp-client-2025-04-04';
const String _anthropicExtendedCacheTtlBeta = 'extended-cache-ttl-2025-04-11';
const String _anthropicFilesApiBeta = 'files-api-2025-04-14';

AnthropicMessagesRequest _buildAnthropicMessagesRequest({
  required String modelId,
  required _AnthropicEncodedPrompt prompt,
  ...
  var maxTokens = options.maxOutputTokens ?? _anthropicDefaultMaxTokens;
  final temperature = _normalizeAnthropicTemperature(
    options.temperature,
    warnings: warnings,
  );
  ...
  if (extendedThinking) {
    var thinkingBudget = providerOptions.thinkingBudgetTokens ??
        ...
        _anthropicDefaultThinkingBudgetTokens;
    if (providerOptions.thinkingBudgetTokens != null &&
    ...
    } else if (thinkingBudget < _anthropicDefaultThinkingBudgetTokens) {
      warnings.add(
        const ModelWarning(
          type: ModelWarningType.compatibility,
          field: 'thinkingBudgetTokens',
      ...
      thinkingBudget = _anthropicDefaultThinkingBudgetTokens;
    }
  ...
  if (interleavedThinking) {
    if (!extendedThinking) {
      ...
      betaFeatures.add(_anthropicInterleavedThinkingBeta);
    }
  }
  ...
  if (mcpServers != null && mcpServers.isNotEmpty) {
    betaFeatures.add(_anthropicMcpClientBeta);
  }

  if (_containsAnthropicCacheControl(body)) {
    betaFeatures.add(_anthropicExtendedCacheTtlBeta);
  }

  if (_containsAnthropicFileSource(body)) {
    betaFeatures.add(_anthropicFilesApiBeta);
  }
...
AnthropicMessagesRequest _buildAnthropicTokenCountRequest({
  required AnthropicMessagesCodec codec,
  required String modelId,
  ...
  final baseRequest = codec.encodeRequest(
    modelId: modelId,
    prompt: prompt,
    tools: tools,
    toolChoice: toolChoice,
  ...
  final body = <String, Object?>{
    ...
    if (baseRequest.body['thinking'] case final thinking?) 'thinking': thinking,
...
double? _normalizeAnthropicTemperature(
  double? value, {
  required List<ModelWarning> warnings,
}) {
  if (value == null) {
...
Map<String, Object?> _anthropicJsonObject(
  Object? value, {
...
bool _containsAnthropicCacheControl(Object? value) {
  if (value is Map) {
    ...
    for (final nestedValue in value.values) {
      if (_containsAnthropicCacheControl(nestedValue)) {
        return true;
      }
    }
    return false;
  ...
  if (value is List) {
    return value.any(_containsAnthropicCacheControl);
  }

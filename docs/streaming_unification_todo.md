...
- [x] Decide: expand current `LLMStreamPart` vs introduce `LLMStreamPartV2`
- [x] Define the "provider tool" representation (typed parts vs `providerExecuted` flag)
- [x] Define `finishReason` unified mapping (or keep raw + best-effort unified)
...
- [x] `openai` + `azure` providers (Responses mode): ensure consistent parts semantics
...
- [x] Add source part tests for OpenAI Responses + Google grounding + xAI citations + Anthropic citations
...
- [x] Update `docs/roadmap.md` MVP 3 status + link this tracker
- [x] Add a migration guide (next alpha) describing new parts + adapter behavior
- [x] Update provider docs to mention typed `source` and provider tool parts
...
- usage + finishReason mapping (toolCalls vs provider-executed tools)

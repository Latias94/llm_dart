...
- Shared capability helper parity now also exists in `llm_dart_core` through `embed(...)`, `embedMany(...)`, `generateImage(...)`, `generateSpeech(...)`, and `transcribe(...)`; embedding migrations now already exist across both OpenAI-family and Google providers through `OpenAI.embeddingModel(...)` and `Google.embeddingModel(...)`, and the OpenAI family now also has package-owned `speechModel(...)` and `transcriptionModel(...)` migrations. The remaining gap is provider-migration parity for image plus the still-incomplete Google/Anthropic speech-related capability coverage, and the still-unfrozen question of whether embeddings later need shared chunk-splitting policy above the raw model interface.

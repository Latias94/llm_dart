...
- OpenAI-family chat migration is closer to the reference now that the chat-completions path accepts user image/audio/PDF file inputs, the Responses-first compatibility route again covers the common user image/file subset, and both OpenAI text paths now align on provider-owned reasoning-model compatibility such as `reasoningEffort`, `forceReasoning`, `systemMessageMode`, and `serviceTier` validation; the main remaining OpenAI-owned gap is now the still-unexposed Responses persistence policy around `store`, `conversation`, and `item_reference`, while assistant replay remains intentionally conservative on the chat-completions path.

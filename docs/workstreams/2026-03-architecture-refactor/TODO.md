...
- [x] Decide that current Google `toolCall` / `toolResponse` UI and replay helpers stay above provider-owned custom replay instead of depending directly on codec-local projection support
...
- [x] Decide that Google streamed TTS remains deferred and compatibility-only for now, instead of reviving the legacy event contract inside the modern package surface
...
- [x] Localize community-provider compatibility config adapters and shell support under provider-owned compatibility directories, leaving only shared Dio override projection in the cross-provider helper

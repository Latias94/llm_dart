...
- `llm_dart_core` now also exposes `OutputSpec`-based `generateOutput(...)` and `streamOutput(...)`, with shared validation errors and a shared streamed-result accumulator above `TextStreamEvent`

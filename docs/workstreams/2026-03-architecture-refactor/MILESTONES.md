...
- the current root `DioClientFactory` now also builds configured clients through the transport-owned Dio setup path instead of routing through `BaseHttpProvider`
...
- the next structural cleanup step is now also clearer: the remaining blocker is no longer cancellation or configurable Dio setup, but compatibility-shaped utility ownership such as root `DioClientFactory` policy and `HttpResponseHandler` error mapping

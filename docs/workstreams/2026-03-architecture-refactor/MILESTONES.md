...
- the next transport-helper extraction step is now also clearer: the remaining blocker is no longer request cancellation, but compatibility-shaped Dio setup and error-mapping utilities such as `HttpConfigUtils`, `DioClientFactory`, and `HttpResponseHandler`

...
- the old broad OpenAI migration umbrella is now also closed at the current workstream scope: the remaining OpenAI items are explicit future provider-owned policy questions around hosted execution and richer helper surfaces, not mainline architecture blockers
- the remaining OpenAI hosted-tool tail is now also moved out of active migration debt: any later replay/helper expansion stays future provider-owned policy only until a concrete product need appears
...
- the shared runner is now also effectively closed at the current scope: further expansion into approval-safe/provider-executed continuation or constrained pre-step mutation remains future demand-driven policy, not current migration debt
...
- the Anthropic compatibility layer is now also explicitly frozen at the current split: thin shell plus one provider-local replay adapter, without more symmetry-driven helper extraction
...
- public Google native-tool selection is now also moved out of active migration debt: the landed Gemini 3 subset proves the provider-owned boundary, while any later forcing or selection API remains future Google-owned policy only
...
- the lightweight `readChatUiStream(...)` helper is now also explicitly frozen on its current result-object contract, so callback-heavy or richer final-summary facades remain future demand-driven policy rather than active runtime migration debt
...
- the root OpenAI provider shell is now also cleaner in ownership terms: the
  remaining provider-owned helper logic for model validation, follow-up
  suggestion generation, suggestion parsing, and helper-level embedding
  delegation now lives in `openai_provider_support.dart`, while
  `provider_compat.dart` stays closer to a pure capability-construction and
  delegation shell
...
- the legacy root config map now has a transitional namespaced `providerOptions` layer: OpenAI-family helpers and the remaining provider-specific builder callbacks write provider-owned values under provider namespaces, while adapters keep namespaced-first, flat-fallback reads during migration; the old broad `ProviderConfig` map helper was later removed in favor of provider-specific builder callbacks and typed provider options

...
- the next structural cleanup step is now also clearer: the remaining blocker is no longer cancellation, provider-facing Dio setup, provider defaults, the shared UTF-8 decoder, or shared log/JSON parsing primitives, but compatibility-shaped config ownership plus the remaining `HttpResponseHandler` error mapping

...
- the next structural cleanup step is now also clearer: the remaining blocker is no longer cancellation, provider-facing Dio setup, provider defaults, or the shared UTF-8 decoder, but compatibility-shaped config ownership plus `HttpResponseHandler` error mapping

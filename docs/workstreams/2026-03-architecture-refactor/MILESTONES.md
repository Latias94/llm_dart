...
- the next structural cleanup frontier is now also clearer: the workspace still has a temporary `llm_dart_core <-> llm_dart_transport` package cycle, `llm_dart_community` is still only a skeleton, and the root package still carries too much local compatibility/provider implementation weight

...
- `llm_dart_core` now also exposes `OutputSpec`-based `generateOutput(...)` and `streamOutput(...)`, with shared validation errors, a shared streamed-result accumulator above `TextStreamEvent`, and best-effort partial structured-output events on the streaming path

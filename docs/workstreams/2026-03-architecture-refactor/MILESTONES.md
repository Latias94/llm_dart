...
- the remaining richer Responses gap is now explicitly narrowed to an OpenAI-owned persistence policy question around `store`, `conversation`, and `item_reference`, not a missing shared-core replay type

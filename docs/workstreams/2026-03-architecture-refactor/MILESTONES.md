...
- the next structural cleanup step is now also clearer: the remaining blocker is no longer cancellation, provider-facing Dio setup, or provider defaults, but compatibility-shaped config ownership plus `HttpResponseHandler` error mapping

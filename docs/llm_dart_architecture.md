...
- **MVP3 (Vercel-style task APIs) — initial cut**:
  ...
  - Implements minimal provider-agnostic tasks:
    ...
    - `streamToolLoopParts` (streaming): stream parts while executing tool calls between steps, yielding a final `LLMFinishPart` once completed
    ...
    - `streamChatParts` (parts-first streaming; recommended)
    - `embed`
    - `generateImage`
  ...
  - Legacy convenience aliases (`*FromPromptIr` / `*FromPrompt`) were removed as part of the fearless refactor cleanup.
...
- **Unified stream parts (Vercel-style) — MVP shipped (initial)**:
  ...
  - Streaming is parts-first: consume `LLMStreamPart` directly to keep one source of truth.
